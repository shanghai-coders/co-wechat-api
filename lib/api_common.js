...
// const fixieUrl = liburl.parse(process.env.FIXIE_URL);
const request = require("request");
const fixieRequest = request.defaults({ proxy: process.env.FIXIE_URL });

const JSONbig = require("json-bigint");
const { replaceJSONCtlChars } = require("./util");
...
class API {
  ...
  async request(url, opts, retry) {
    ...
    var res = await makeRequest(url, options);

    // const requestUrl = liburl.parse(url);
    ...
    //           "Proxy-Authorization": `Basic ${new Buffer(
    //             fixieUrl.auth
    //           ).toString("base64")}`
    //         },
    //         ...options
    ...
    if (contentType.indexOf("application/json") !== -1) {
      ...
      if (data && data.errcode) {
        ...
        if (
          (err.code === 40001 || err.code === 42001) &&
          retry > 0 &&
          !this.tokenFromCustom
        ) {
          // 销毁已过期的token
